...
def get_questions(start, end):
    ...
    with conn:
        sql = " SELECT * FROM answers LIMIT " + end
...
def add_question_func(question_text, parameter1_text, parameter2_text, keyword1_text, keyword2_text, detailedanswer_text):
    conn = create_connection()

    with conn:
        cur = conn.cursor()
        # sql ="SELECT * FROM users WHERE name='" + username +"' AND password='"+password+"'"
        sql  = ''' INSERT INTO answers(question)
                          VALUES(?) '''
                          # VALUES(?,?) '''
        sql  = "INSERT INTO answers(question, parameter1, parameter2, keyword1, keyword2, detailedanswer) " \
               "VALUES('"+ question_text +"','"+parameter1_text+"','"+parameter2_text+"','"+keyword1_text+"','"+keyword2_text+"','"+detailedanswer_text+"')  "
        try:
...
def genaric_algo(selected_items, selected_keywords):
    """ Create Connection"""
    conn = create_connection()
    with conn:
        cur = conn.cursor()
...
def genaric_keywords_iteration(cur, selected_item_1, selected_item_2, selected_keywords ):
    for i in range(len(selected_keywords)):
        for j in range(i + 1, len(selected_keywords)):
            sql =" SELECT detailedanswer from answers_null WHERE parameter1 = '"+ selected_item_1 +"' AND parameter2 = '"+ selected_item_2 +"' and keyword1 = '"+selected_keywords[i]+"' AND keyword2 = '"+selected_keywords[j]+"'"
            cur.execute(sql)
            rows = cur.fetchall()
            if len(rows)> 0:
                return (rows[0])[0]
            sql =" SELECT detailedanswer from answers_null WHERE parameter1 = '"+ selected_item_1 +"' AND parameter2 = '"+ selected_item_2 +"' and keyword1 = '"+selected_keywords[j]+"' AND keyword2 = '"+selected_keywords[i]+"'"
            cur.execute(sql)
            rows = cur.fetchall()
            if len(rows)> 0:
                return (rows[0])[0]
...
# var_qn = get_questions('start','end')
# questions_dict = {i: var_qn[i] for i in range(0, len(var_qn))}
...
# while True:
#     qn = input('Q: ')
#     back = add_question('q','p1','p2','k1','k2','da')
#     print(back)